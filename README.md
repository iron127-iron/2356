...
> **Note:** Layer4 methods (SYN, ICMP, AMP, OVH-UDP) require **root** on Android.
> Install `tsu` and run `tsu` before using those methods.
> Layer7 methods (GET, POST, CFB, etc.) work without root.
...
## Android APK Build (Windows 11)

On Windows 11, use **WSL2 + Docker Desktop** to build the APK:
...
# 3. Reboot, then run the build script (as Administrator)
.\build-apk.ps1
```

**Alternative: GitHub Actions (no local setup needed)**
1. Push this repo to GitHub
2. Go to Actions > Build Android APK > Run workflow
3. Download the APK from the completed workflow artifacts

**Manual build in WSL2:**
```bash
# Inside WSL2 (Ubuntu)
cd /mnt/c/Users/<user>/Desktop/MHDDoS-2.4.4
bash docker-build-apk.sh
```

The APK will be at `bin/mhddos-*-debug.apk`.

> **APK Limitations:**
> - Layer7 methods (GET, POST, CFB, etc.) work without root
> - Layer4 raw socket methods (SYN, ICMP, AMP) require root - not available in standard APK
> - The app has a simple Kivy GUI for entering attack parameters
> - Real-time output is shown in the app log
